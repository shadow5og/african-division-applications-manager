...
# RUN \
#   if [ -f yarn.lock ]; then yarn --frozen-lock-file; \
...
COPY --from=builder --chown=nextjs:nodejs /app/key.json ./
COPY --from=builder --chown=nextjs:nodejs /app/application_default_credentials.json ./
